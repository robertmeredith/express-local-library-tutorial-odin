  ...
  async.parallel(
    {
      ...
      books_bookinstances: function (callback) {
        BookInstance.findById({ book: req.body.bookid }).exec(callback);
      },
    },
    function (err, results) {
      if (err) {
      ...
      if (results.books_bookinstances.length > 0) {
        ...
        res.render('book_delete', {
          title: 'Delete Book',
          book: results.book,
          book_bookinstances: books_bookinstances,
        });
      } else {
        // Book has no books.  Delete object and redirect to list of Books
